...
public class TriangularDistribution {
	...
	public TriangularDistribution(int a, int c, int b) {
		if (a < c && c < b){
			...
			System.out.println("The condition: a < c AND c < b, does NOT hold!");
			this.a = 0;
			this.c = 50;
			this.b = 100;
		}
	...
	public Rational pdf(int x) {
		...
		if (x < a){
			Rational prob = new Rational(0,0);
			return prob;
		}
		else if (a <= x && x < c){
			Rational prob = new Rational(2*(x-a),(b-a)*(c-a));
			return prob;
		}
		else if (x==c){
			Rational prob = new Rational(2,(b-a));
			return prob;
		}
		else if (c < x && x <= b){
			Rational prob = new Rational(2*(b-x),(b-a)*(b-c));
			return prob;
		}
		else if(b < x){
			Rational prob = new Rational(0,0);
